...
const MRP_MO_ItemTabNotes = () => {
  ...
  const fetchMoData = async () => {
    setIsLoading(true)
    try {
      const response = await fetch(`${apiUrl}/mos/${moId}`, {
        headers: {
          DOLAPIKEY: apiKey
        }
      })
          ...
          <Grid item>
            <Box display='flex' alignItems='center' gap={2}>
              <Box
                ...
                overflow='hidden'
              >
                <img width={64} height={64} src='https://f.start.me/us.gov' alt='MO' />
              </Box>
              <Typography variant='h5' component='div'>
                {moData.ref || 'No Reference'}
              </Typography>
            </Box>
          </Grid>
          <Grid item>
            <Link href='/mrp/mo/list' passHref>
...
export default MRP_MO_ItemTabNotes
